...
**DNA sample:** Peripheral venous blood
...
var_tbl <- read.csv("MSIseq_out/Input_vars.csv", row.names = 1)
rownames(var_tbl) <- ""
var_tbl <- round(var_tbl, 2)
pander(var_tbl, align = "c", table.caption.prefix ="")
```

- T.sns: total count of SNVs/Mb

- S.sns: count of SNVs in simple sequence repeats/Mb
...
class_tbl <- read.csv("MSIseq_out/result.csv", row.names = 1)
rownames(class_tbl) <- ""
class_tbl$Tumor_Sample_Barcode <- NULL
