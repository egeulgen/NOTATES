...
# II. Glioblastoma Molecular Classification (Verhaak et al.)

Glioblastoma can be stratified into four transcriptional subtypes based on RNA quantification results: Proneural, Neural, Classical, and Mesenchymal—based on gene expression profiling as described by Verhaak et al. [^3] These subtypes are associated with distinct genomic alterations and biological characteristics: the Proneural subtype is commonly linked to PDGFRA abnormalities and IDH1 mutations; the Classical subtype is characterized by EGFR amplification and chromosome 7 gain/10 loss; the Mesenchymal subtype is associated with NF1 loss and inflammatory signatures; and the Neural subtype reflects expression patterns of normal neuronal tissue. This classification captures the molecular heterogeneity of GBM and has been shown to correlate with differences in tumor biology and therapeutic response.
...
Below are the calculated scores for each subtype for this tumor.

\begin{center}
\includegraphics[width=400pt]{RNAseq_results/verhaak_classification/`r paste0(params$rnaseq_sample_id, "_verhaak_scores.pdf")`}
\end{center}


\newpage

# III. Germline Alterations
...
## III. a. ACMG Incidental Findings
...
## III. b. Variants in Cancer Gene Census Genes
...
## III. c. Variants in Cancer Predisposition Genes

Genes in which germline mutations confer highly or moderately increased risks of cancer are called cancer predisposition genes. Filtered germline SNV/indels cataloged in the 2014 Nature publication by Rahman [^4], studying cancer predisposition genes, are reported in this subsection.

[^4]: Rahman N. Realizing the promise of cancer predisposition genes. Nature. 2014;505(7483):302-8.
...
## III. d. Variants in DNA Damage Repair Genes

In this subsection, filtered germline variants in Human DNA Repair Genes [^5] are presented.

[^5]: Mdanderson.org. 2020. Human DNA Repair Genes. [online] Available at: <https://www.mdanderson.org/documents/Labs/Wood-Laboratory/human-dna-repair-genes.html>.
...
`r if(params$type == "glioma") {"## III. e. Common Variants"}`
...
# IV. Somatic Single Nucleotide Variations (SNVs) and Small Insertion/Deletions (Indels)
...
## IV. a. Tumor Mutational Burden (TMB)
...
## IV. b. Microsatellite Instability Status

Microsatellite instability (MSI) is the result of impaired DNA mismatch repair and constitutes a cellular phenotype of clinical significance in many cancer types. The MSI status of the tumor was predicted using the tool MSIpred [^6]. MSIpred computes 22 somatic mutational features (predictors of MSI) and using these features, it predicts tumor MSI status as microsatellite instability high (MSI-H) or microsatellite stable (MSS) using a machine learning method, support vector machine. 
...
[^6]: Wang C, Liang C. MSIpred: a python package for tumor microsatellite instability classification from tumor mutation annotation data using a support vector machine. Sci Rep. 2018;8(1):17546.
...
`r if(params$type == "glioma") {"## IV. c. Variants in Established Glioma Genes"} else {"## IV. c. Variants in Established Medulloblastoma Genes"}`

`r if(params$type == "glioma") {"This subsection contains somatic SNV/indels in genes that have been reported in the TCGA pan-glioma study of Ceccarelli et al. [^7] which analyzed 1122 WHO grade II-III and IV diffuse-gliomas."} else {"This is the intersection of somaticSNV/indels in this case with a list of genes manually curated because of their importance in medulloblastomas."}`

`r if(params$type == "glioma") {"[^7]: Ceccarelli M, Barthel FP, Malta TM, et al. Molecular Profiling Reveals Biologically Discrete Subsets and Pathways of Progression in Diffuse Glioma. Cell. 2016;164(3):550-63."}`
...
if(file.exists("NOTATES/Somatic_SNV/important_SNVs.csv")) {
  ...
  colnames(imp_som_var) <- c("Gene", "Variant Classification", "Protein", "Genomic Change", "VAF", "COS_n",
                         "UniProt_Region", "Irino", "TMZ")
  
  imp_som_var$COSMIC <- imp_som_var$Gene
  cosmic_urls <- paste0("https://cancer.sanger.ac.uk/cosmic/gene/analysis?ln=", imp_som_var$Gene)
  
  
  k_tbl <- kable(imp_som_var, format = "latex", booktabs = TRUE, longtable = TRUE)
  k_tbl <- column_spec(k_tbl, which(colnames(imp_som_var) == "UniProt_Region"), width = "6em")
  k_tbl <- column_spec(k_tbl, which(colnames(imp_som_var) == "COSMIC"), link = cosmic_urls)
  kable_styling(k_tbl, latex_options = c("hold_position", "repeat_header"), position = "center", font_size = 8)
} else {
  cat("No variation to report.")
}
```

## IV. d. Hotspot Variants in Cancer Gene Census Genes
...
if(file.exists("NOTATES/Somatic_SNV/COSMIC_hotspot.csv")) {
  ...
  hot_som <- hot_som[, setdiff(colnames(hot_som), 
                               c("COSMIC_total_alterations_in_gene", "DNA_repair", "selected_KEGG"))]
  colnames(hot_som) <- c("Gene", "Variant Classification", "Protein", "Genomic Change", "VAF", "COS_n",
                         "UniProt_Region","Irino", "TMZ")
  
  hot_som$COSMIC <- hot_som$Gene
  cosmic_urls <- paste0("https://cancer.sanger.ac.uk/cosmic/gene/analysis?ln=", hot_som$Gene)
  
  k_tbl <- kable(hot_som, format = "latex", booktabs = TRUE, longtable = TRUE)
  k_tbl <- column_spec(k_tbl, which(colnames(hot_som) == "UniProt_Region"), width = "6em")
    k_tbl <- column_spec(k_tbl, which(colnames(hot_som) == "COSMIC"), link = cosmic_urls)
  kable_styling(k_tbl, latex_options = c("hold_position", "repeat_header"), position = "center", font_size = 8)
} else {
  cat("No variation to report.")
}
...
## IV. e. Other Variants in Cancer Gene Census Genes
...
if(file.exists("NOTATES/Somatic_SNV/CGC_genes.csv")) {
  ...
  cgc_som <- cgc_som[, setdiff(colnames(cgc_som), 
                               c("COSMIC_total_alterations_in_gene", "DNA_repair", "selected_KEGG"))]
  colnames(cgc_som) <- c("Gene", "Variant Classification", "Protein", "Genomic Change","VAF", "COS_n",
                         "UniProt_Region", "Irino", "TMZ")
  
  cgc_som$COSMIC <- cgc_som$Gene
  cosmic_urls <- paste0("https://cancer.sanger.ac.uk/cosmic/gene/analysis?ln=", cgc_som$Gene)
  
  k_tbl <- kable(cgc_som, format = "latex", booktabs = TRUE)
  k_tbl <- column_spec(k_tbl, which(colnames(cgc_som) == "UniProt_Region"), width = "6em")
  k_tbl <- column_spec(k_tbl, which(colnames(cgc_som) == "COSMIC"), link = cosmic_urls)
  kable_styling(k_tbl, latex_options = c("hold_position", "scale_down"), position = "center", font_size = 8)
} else
{
  cat("No variation to report.")
}
...
## IV. f. Other Possibly Important Somatic SNV/indels
...
if(file.exists("NOTATES/Somatic_SNV/DDR_related.csv")) {
  ...
  colnames(ddr_som_var) <- c("Gene", "Variant Classification", "Protein", "Genomic Change", "VAF", "COS_n",
                             "UniProt_Region", "DNA_repair", "Irino", "TMZ")
  
  ddr_som_var$COSMIC <- ddr_som_var$Gene
  cosmic_urls <- paste0("https://cancer.sanger.ac.uk/cosmic/gene/analysis?ln=", ddr_som_var$Gene)
  
  k_tbl <- kable(ddr_som_var, format = "latex", booktabs = TRUE, longtable = TRUE)
  k_tbl <- column_spec(k_tbl, which(colnames(ddr_som_var) == "UniProt_Region"), width = "6em")
  k_tbl <- column_spec(k_tbl, which(colnames(ddr_som_var) == "COSMIC"), link = cosmic_urls)
  kable_styling(k_tbl, latex_options = c("hold_position", "repeat_header"), position = "center", font_size = 8)
} else {
  cat("No variation to report.")
}
...
if(file.exists("NOTATES/Somatic_SNV/KEGG_selected.csv")) {
  ...
  colnames(kegg_som_var) <- c("Gene", "Variant Classification", "Protein", "Genomic Change", "VAF", "COS_n",
                              "COS_tot", "UniProt_Region", "DNA_repair", "Irino", "TMZ", "KEGG")
  
  kegg_som_var$COSMIC <- kegg_som_var$Gene
  cosmic_urls <- paste0("https://cancer.sanger.ac.uk/cosmic/gene/analysis?ln=", kegg_som_var$Gene)
  ...
  make_table <- function(df) {
    ...
    if ("UniProt_Region" %in% names(df)) {
      k_tbl <- column_spec(
        k_tbl,
        which(names(df) == "UniProt_Region"),
        width = "6em"
      )
    }
  
    if ("COSMIC" %in% names(df)) {
      k_tbl <- column_spec(
        k_tbl,
        which(names(df) == "COSMIC"),
        link = cosmic_urls
      )
    }
  ...
  common_cols <- c(
    "Gene", "Variant Classification", "Protein",
    "Genomic Change", "VAF"
  )
  
  tbl1 <- make_table(kegg_som_var[, c(
    common_cols,
    "COS_n", "COS_tot", "COSMIC", "Irino", "TMZ"
  )])
  
  tbl2 <- make_table(kegg_som_var[, c(
    common_cols,
    "UniProt_Region", "DNA_repair",
    "KEGG"
  )])
  
  cat(as.character(tbl1))
  cat("\n\n")
...
# V. Somatic Copy Number Alterations (SCNAs)

Only SCNAs with a $log_2(Tumor/Normal)$ ratio $\le$ -0.25 or $\ge$ 0.25 were used in analysis.

## V. a. SCNA Burden

Numerous studies have shown that SCNA burden is an important prognostic marker ^[^8],^[^9]^,^[^10]. Below are 4 metrics associated with SCNA Burden:

[^8]: Hieronymus H, Murali R, Tin A, et al. Tumor copy number alteration burden is a pan-cancer prognostic factor associated with recurrence and death. Elife. 2018;7
[^9]: Hieronymus H, Schultz N, Gopalan A, et al. Copy number alteration burden predicts prostate cancer relapse. Proc Natl Acad Sci USA. 2014;111(30):11139-44.s
[^10]: Zhang L, Feizi N, Chi C, Hu P. Association Analysis of Somatic Copy Number Alteration Burden With Breast Cancer Survival. Front Genet. 2018;9:421.
...
`r if(params$type == "glioma") {"## V. b. Established SCNAs in Glioma"} else {"## V. b. Established SCNAs in Medulloblastoma"}`
...
## V. c. SCNAs in Cancer Gene Census Genes
...
# VI. Somatic Translocations

## VI. a. Events from DNA

Possible somatic translocations based on DNA are called using `DELLY`. Only gene-to-different-gene structural variants (where both ends of the structural variant are in the exonic/intronic regions of the two genes) are kept. Below is the circos plot displaying possible translocations.
...
## VI. b. Events from RNA

Possible somatic translocations based on RNA are called using `arriba`. Below is the circos plot displaying possible translocations.

\begin{center}
\includegraphics[width=400pt]{RNAseq_results/fusions/`r paste0(params$rnaseq_sample_id, "_circos.pdf")`}
\end{center}

```{r translocation_table_rna}
transloc_df <- read.delim("RNAseq_results/fusions/NOT-0123-Tumor.fusions.tsv")

if(nrow(transloc_df) != 0) {
  colnames(transloc_df)[1] <- "gene1"
  
  selected_cols <- c("gene1", "gene2", "type", "confidence")
  transloc_df <- transloc_df[, selected_cols]
  ...
  transloc_df <- data.frame(
    lapply(transloc_df, function(x) {
      x <- escape_latex(x)
      x <- gsub(",", ",\\\\allowbreak ", x)
      x
    })
  )
  k_tbl <- kable(transloc_df,
                format = "latex", 
                booktabs = TRUE,
                align = "c",
                longtable = TRUE,
                escape = FALSE)
  k_tbl <- column_spec(k_tbl, 1:4, width = "10em")
...
# VII. Loss of Heterozygosity (LOH) Events
...
if (nrow(loh) == 0) {
  ...
  kable_styling(kable(loh,
                ...
                latex_options = c("hold_position", "repeat_header"), position = "center")
}
```

## VII. b. LOH + Somatic SNV/Indel
...
if(file.exists("NOTATES/LOH/d_hit_loh.csv"))
{
  ...
  k_tbl <- kable(loh_dhit, format = "latex", booktabs = TRUE, align = "c", longtable = TRUE)
  kable_styling(k_tbl, latex_options = c("hold_position", "repeat_header"), position = "center")
}else
{
...
## VII. c. LOH Events in Cancer Gene Census Genes 
...
# VIII. Genes with Double Hit

A double hit strongly suggests a relevant tumor-suppressor gene [^11]. In this section, the list of genes with somatic SNV/indel(s) as well as SCNA and/or LOH events are reported.

[^11]: Vogelstein B, Papadopoulos N, Velculescu VE, Zhou S, Diaz LA, Kinzler KW. Cancer genome landscapes. Science. 2013;339(6127):1546-58.

```{r dhit_tbl}
if(file.exists("NOTATES/double_hit.csv"))
{
  dhit_tbl <- read.csv("NOTATES/double_hit.csv")
  k_tbl <- kable(dhit_tbl, format = "latex", booktabs = TRUE, align = "c", longtable = TRUE)
  kable_styling(k_tbl, latex_options = c("hold_position", "repeat_header"), position = "center")
} else {
  cat("No event to report.")
}
...
# IX. Tumor Heterogeneity Analysis 
...
# X. Tumor Immune Contexture

The immune composition of the tumor was inferred using quanTIseq [^13], a computational method for deconvoluting bulk RNA-seq data to estimate the relative fractions of ten immune cell types, including T cells, B cells, NK cells, and myeloid populations. This approach provides a quantitative view of the tumor immune microenvironment, which can inform on tumor-immune interactions and potential therapeutic vulnerabilities.

[^13]: Finotello F, Mayer C, Plattner C, Laschober G, Rieder D, Hackl H, Krogsdam A, Loncova Z, Posch W, Wilflingseder D, Sopper S, Ijsselsteijn M, Brouwer TP, Johnson D, Xu Y, Wang Y, Sanders ME, Estrada MV, Ericsson-Gonzalez P, Charoentong P, Balko J, de Miranda NFDCC, Trajanoski Z. Molecular and pharmacological modulators of the tumor immune contexture revealed by deconvolution of RNA-seq data. Genome Medicine, 2019. 11(1):34.

\begin{center}
\includegraphics[width=400pt]{RNAseq_results/quantiseqr/`r paste0(params$rnaseq_sample_id, "_quanti_plot.pdf")`}
\end{center}


Additionally, single sample Gene Set Enrichment (GSEA) was performed using RNAseq quantification results to investigate any enrichment in "C7: immunologic signature gene sets".


\begin{center} 
\includegraphics[width=400pt]{RNAseq_results/gene_set_enrichment/`r paste0(params$rnaseq_sample_id, "_immunological_scores.pdf")`}
\end{center}

\newpage

# XI. Mutational Signatures

Somatic mutations in cancer genomes are caused by multiple mutational processes each of which generates a characteristic mutational signature (i.e. a distinct pattern of substitution types in specific sequence contexts) [^12]. Importantly, recent studies show that mutational signatures could have significant clinical impact in certain tumor types [^14]^-^[^15].

[^12]: Ludmil B Alexandrov, Jaegil Kim, Nicholas J Haradhvala, Mi Ni Huang, Alvin WT Ng, Yang Wu, Arnoud Boot, Kyle R Covington, Dmitry A Gordenin, Erik N Bergstrom, S M Ashiqul Islam, Nuria Lopez-Bigas, Leszek J Klimczak, John R McPherson, Sandro Morganella, Radhakrishnan Sabarinathan, David A Wheeler, Ville Mustonen, the PCAWG Mutational Signatures Working Group, Gad Getz, Steven G Rozen, Michael R Stratton. The Repertoire of Mutational Signatures in Human Cancer. bioRxiv (2019). doi: 10.1101/322859
[^14]: Dong F, Davineni PK, Howitt BE, Beck AH. A BRCA1/2 Mutational Signature and Survival in Ovarian High-Grade Serous Carcinoma. Cancer Epidemiol Biomarkers Prev. 2016;25(11):1511-1516.
[^15]: Secrier M, Li X, De silva N, et al. Mutational signatures in esophageal adenocarcinoma define etiologically distinct subgroups with therapeutic relevance. Nat Genet. 2016;48(10):1131-41.
...
colnames(mut_sigs) <- c("Freq", "Aetiology", "Associated", "Comments")
k_tbl <- kable(mut_sigs, format = "latex", booktabs = TRUE, longtable=TRUE)
k_tbl <- column_spec(k_tbl, 3, width = "10em")
k_tbl <- column_spec(k_tbl, 4, width = "10em")
k_tbl <- column_spec(k_tbl, 5, width = "10em")
kable_styling(k_tbl, latex_options = c("repeat_header"), position = "center")
```

\newpage

# XII. Gene Set Enrichment Analysis

## XII. a. Whole Transcriptime Sequencing - Gene Set Enrichment Analysis

Gene set enrichment analysis is frequently utilized for studying the mechanisms underlying oncological processes. For this analysis, Hallmark gene sets (H),  oncogenic signature gene sets (C6) and KEGG pathway gene sets enrichment analysis was performed using the Single-sample Gene Set Enrichment Analysis (ssGSEA) method.

\begin{center} 
\includegraphics[width=400pt]{RNAseq_results/gene_set_enrichment/`r paste0(params$rnaseq_sample_id, "_hallmark_scores.pdf")`}
\end{center}


\begin{center} 
\includegraphics[width=400pt]{RNAseq_results/gene_set_enrichment/`r paste0(params$rnaseq_sample_id, "_oncogenic_scores.pdf")`}
\end{center}


\begin{center} 
\includegraphics[width=400pt]{RNAseq_results/gene_set_enrichment/`r paste0(params$rnaseq_sample_id, "_KEGG_scores.pdf")`}
\end{center}


## XII. b. Whole Exome Sequencing - Gene Set Enrichment Analysis
...
### XII. b1. Enrichment Results for High-impact Somatic SNV/indels
...
if (is.data.frame(enr_res)) {
  selected_col_idx <- c(2, 8)
  enr_res <- enr_res[, selected_col_idx]
  colnames(enr_res) <- c("Pathway", "Somatic SNV/indel")
...
### XII. b2. Enrichment Results for High-impact SCNA
...
if (is.data.frame(enr_res)) {
  selected_col_idx <- c(2, 8, 9)
  enr_res <- enr_res[, selected_col_idx]
  colnames(enr_res) <- c("Pathway", "Amplification", "Deletion")
