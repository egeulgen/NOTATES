...
2. keeping variants with a sequence depth greater than 20X in the tumor sample greater and 10X in the normal sample and 
3. excluding variants short tandem repeat (STR) variants

```{r TMB, echo=F, fig.width=12, fig.height=5}
...
somatic_SNVs <- somatic_SNVs[somatic_SNVs$alt_allele_seen == "True" &
                               somatic_SNVs$short_tandem_repeat_membership == "False", ]

# subset for tumor_f > 0.05, depth
...
TMB_tbl_path <- file.path(params$script_dir, 
                          paste0(params$type, "_TMB_table.txt"))
if (file.exists(TMB_tbl_path)) {
  TMB_df <- read.delim(TMB_tbl_path, sep = "\t") 
  ...
  if(!params$ID %in% TMB_df$ID) {
    ...
    TMB_df <- rbind(TMB_df, data.frame(ID = params$ID, 
                                       TMB = current_TMB, 
                                       subset = current_sset,
                                       primary = params$primary_tm))
    write.table(TMB_df, 
                file.path(params$script_dir, "TMB_table.txt"), 
...
> Mutation load (Number of mutations per Mb) for the current tumor sample is: `r mut_load` 
