...
header-includes:
- \usepackage{titling}
- \pretitle{\begin{center}\LARGE\includegraphics[width=90pt]{`r paste0(params$script_dir, "/NOT_logo.png")`}\\[\bigskipamount]}
...
# I. Quality Metrics
```{r QC, echo = F}
QC_df <- read.csv("./QC_table.csv",stringsAsFactors = F, row.names = 1)
kable(QC_df, align = "c")
```
...
{
  imp_snv <- read.csv("./NOTATES/Somatic_SNV/important_SNVs.csv", stringsAsFactors = F, colClasses = c("character"))
...
{
  loh_cgc <- read.csv("./NOTATES/LOH/CGC_loh.csv", stringsAsFactors = F)
