...
$GATK VariantFiltration -R $genome -V ./raw_snps.vcf \
	--filter-expression "QD < 2.0 || MQ < 40.0 || FS > 60.0 || SOR > 4.0 || MQRankSum < -12.5 || ReadPosRankSum < -8.0" \
	--filter-name "HQ_SNP_filter" -O ./germline_snps.vcf
...
$GATK SelectVariants -R $genome -V ./raw.snps.indels.vcf \
	--select-type-to-exclude SNP -O ./raw_indels.vcf

echo "################################## Filtering Germline nonSNPs    " $(date)
$GATK VariantFiltration -R $genome -V ./raw_indels.vcf \
	--filter-expression "QD < 2.0 || ReadPosRankSum < -20.0 || FS > 200.0 || SOR > 10.0" \
	--filter-name "HQ_InDel_filter" -O ./germline_indels.vcf

rm ./raw_indels.vcf
rm ./raw_indels.vcf.idx

echo "########### Merging Germline SNP and InDels into a single VCF    " $(date)
$JAVA "$resources_dir""/Tools/GenomeAnalysisTK.jar" -T CombineVariants -R $genome --variant ./germline_snps.vcf \
